...
export default function HousingDetailPage({ params }: PageProps) {
	...
	const housing =
		HOUSING_OPTIONS.find((h) => h.id.toString() === params.houseId) ||
		HOUSING_OPTIONS[0];
	...
	const handleApply = () => {
		if (!message) {
			toast({
				...
				description:
					'A brief introduction helps landlords understand your needs',
				variant: 'destructive',
			});
			return;
		}
		...
		toast({
			...
			description:
				"Your application has been sent to the landlord. You'll receive a response soon.",
		});
	...
	const handleBook = () => {
		// Redirect to payment page with housing details
		router.push(
			`/payment?type=housing&id=${housing.id}&title=${encodeURIComponent(
				housing.title
			)}&amount=${housing.price}`
		);
	};
								...
								<Card>
									...
									<CardContent>
										...
										<ul className="list-disc pl-5 space-y-1 text-gray-600">
											...
											<li>
												Public transportation: 0.2 miles
											</li>
					...
					<div className="space-y-6">
						<Card className="border-0 shadow-lg overflow-hidden">
							...
							<CardHeader>
								...
								<CardDescription>
									Security deposit: ${housing.securityDeposit}
								</CardDescription>
							</CardHeader>
							<CardContent className="space-y-4">
								<div>
									<Label htmlFor="lease-term">
										Lease Term
									</Label>
									<select
										id="lease-term"
